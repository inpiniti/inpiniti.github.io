...
```
  ...
  네 이렇게 작성할 수 있겠죠 여기서 이 콜백 감소가 볼 수 있는지 짝수
  6:44
  인지 판별 해주는 역할을 해야 되는데 뭐 사실 굉장히 단순한 오즈 키
  6:48
  진한데 지금 당장 코드를 만들 형편이 되지 않는다면 이목 함수를 쓰는
  6:53
  거에요
  6:57
  이렇게 쓰는 겁니다 5 캄 수는 잔 수님 투 와 펄스를 번갈아가면서 리턴
  7:02
  해 주도록 하겠습니다
  7:10
  222
  7:29
  네 이렇게 적성 했구요 뭐 이렇게 배열을 확인할 때는 2b 가 아니라 투
  7:34
  스틱 어 를 사용하셔야 됩니다
  7:37
  원래는 이 부분에 다 가 볼 수 있는지 짝수 인지 팜 배를 줄 수 있는
  7:41
  함수를 작성을 해야 되는데
  7:43
  일단은 목 함수를 사용하고 어떤 숫자가 넣어둔 질 그냥 순서대로 두루 설
  7:48
  수트로 풀수 이렇게 번갈아 하면서 1차 늘 해주고 있습니다
  7:51
  실행을 해보면 1 3 50,000
  7:54
  이부분이 투어가 될 것이기 때문에 그래서 리젤 트는
  7:58
  135 가 될 겁니다 한번 해볼게요
  8:03
  네 잘 나오네요 2 제목 함수 대신에 시트에 코트를 작성하시면 되겠습니다
  8:10
  여기서 뭐 리턴 별로 말고
  8:16
  목 리얼 우드 밸리로 사용하면 비동기 함수 를 낼 수도 있습니다
  8:22
  아
  8:24
  아
  ...
  아
  8:52
  네 이렇게 b 동 잠시 처럼 댄 을 이용해서 여기서 l 내주는 값을 드뎌
  8:57
  볼 수 있습니다
  8:59
  이번에는 이렇게 있는 외부 코드를 활용한 테스트가 피라도 과정을 한번
  9:04
  해보겠습니다
  9:09
